...
def update_weather(id, temp_c=None, humidity=None):
    with get_connection() as conn:
        ...
        if temp_c is not None:
            cursor.execute("UPDATE weather_data SET temp_c = ? WHERE id = ?", (temp_c, id))
        if humidity is not None:
            cursor.execute("UPDATE weather_data SET humidity = ? WHERE id = ?", (humidity, id))
        conn.commit()

def delete_weather(id):
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("DELETE FROM weather_data WHERE id = ?", (id,))
        conn.commit()
...
def update_sale(id, umbrellas=None, cold_drinks=None):
    with get_connection() as conn:
        ...
        if umbrellas is not None:
            cursor.execute("UPDATE sales_data SET umbrellas = ? WHERE id = ?", (umbrellas, id))
        if cold_drinks is not None:
            cursor.execute("UPDATE sales_data SET cold_drinks = ? WHERE id = ?", (cold_drinks, id))
        conn.commit()

def delete_sale(id):
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("DELETE FROM sales_data WHERE id = ?", (id,))
        conn.commit()
